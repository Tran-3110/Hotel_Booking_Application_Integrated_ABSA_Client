...
export default function HotelManagement() {
    ...
    const fetchHotels = async () => {
        setLoading(true);
        try {
            const res = await hotelAdminService.getHotelList(page, size);
    ...
    return (
        <div className="p-6 mt-18 max-w-7xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                ...
                <button className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm active:scale-95">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            <div className="flex-shrink-0 h-12 w-12 rounded-lg overflow-hidden border border-gray-200">
                                                <img className="h-12 w-12 object-cover" src={hotel.thumbnail} alt={hotel.name} />
                                            </div>
                                            <div className="ml-4">
                                                <div className="text-sm font-medium text-gray-900">{hotel.name.length > 40 ? hotel.name.substring(0, 40)+"..." : hotel.name}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4">
                                        <div className="text-sm text-gray-900 line-clamp-1">{hotel.street}</div>
                                        <div className="text-sm text-gray-500">{hotel.ward}, {hotel.province}</div>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center">
                                            <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                {hotel.viewCount} views
                                            </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-center">
